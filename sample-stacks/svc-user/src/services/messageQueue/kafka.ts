  ...
  public async publishEvent(event: Event): Promise<boolean> {
    ...
    try {
      ...
      switch (event.type) {
        ...
        case EventType.USER_PREFERENCES_CHANGED:
          topic = `user-${event.type.toLowerCase()}`;
          break;
        default:
          topic = 'user-events';
      }
    ...
    try {
      ...
      await this.consumer.subscribe({
        topics: [
          ...
          'user-user_created',
          'user-user_updated',
          'user-user_preferences_changed',
          // Also subscribe to order topics that are relevant to users
          'order-order_created',
